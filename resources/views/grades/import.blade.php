...
<div class="p-6 rounded overflow-hidden shadow-lg border">
    <div class="font-bold text-xl mb-4">Import grade</div>
    @if (session('gradeImportSuccess'))
    <div class="bg-green-100 text-green-700 py-2 px-4 rounded my-4" role="alert">
        <span class="block sm:inline">{{ session('gradeImportSuccess') }}</span>
    </div>
    @elseif (session('gradeImportFailure'))
    <div class="bg-red-100 text-red-700 py-2 px-4 rounded my-4" role="alert">
        <span class="block sm:inline">{{ session('gradeImportFailure') }}</span>
    </div>
    @endif
    <form class="w-full" action="{{ route('grades.import_csv') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="flex -mx-3">
