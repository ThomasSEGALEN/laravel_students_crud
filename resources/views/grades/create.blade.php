...
<div class="p-6 rounded overflow-hidden shadow-lg border">
    <div class="font-bold text-xl mb-4">Add grade</div>
    @if (session('gradeCreateSuccess'))
    <div class="bg-green-100 text-green-700 py-2 px-4 rounded my-4" role="alert">
        <span class="block sm:inline">{{ session('gradeCreateSuccess') }}</span>
    </div> @endif
    <form class="w-full" action="{{ route('grades.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="flex flex-col md:flex-row -mx-3">
            <div class="w-full md:w-1/2 px-3 md:mb-6">
                ...
                <input
                    ...
                    id="wordingInput" type="text" name="wording" value="{{ old('wording') }}">
                @error('wording')
